...
- **外勤/Duty 改派展示姓名**：审批详情映射 `assigneeName`，「改派」显示「改派给 张三」，不再拼接 `#adminId`。

- **Duties 页文案**：去掉页内「门店 Duties」标题；Tab 改为「店班检查 / 外勤检查 / 明细」；店班检查日历图例与格子不再展示「已跳过」。
...
- **每个店班格子都有「点击分派」**：固定委派与按日委派格子，无论是否已有人员，都显示灰色「点击分派」；点击格子打开对应委派弹窗。外勤模板仍不显示（随工单接单人）。

- **委派入口恢复最早样式**：按日委派格恢复灰色小字「点击分派」；已有人员时也保留这行字，方便再次委派。

- **按日委派入口常显「点击分派」**：日历格在已有委派人员时仍显示「点击分派」文案，避免分配后找不到重新委派入口。

- **修复店班按日委派「分配后无法再分配」**：委派日历格子在已有人员后，员工 Tag 的点击被做成跳转「完成明细」并 `stopPropagation`，挡住了格子上的重新委派。现按日委派模式下 Tag 不再拦截点击，点击格子/Tag 仍打开委派弹窗；固定委派 Tag 仍可跳转完成明细。

- **Duties「完成明细」Tab**：周日期表头可点击切换（默认今天，显示完成数/总数），下方列表展示选中日明细与照片；支持类型/状态/员工筛选。后端 `completions` 支持 `from`/`to`，返回 `workDate`。

## 2026-07-22

- **Duties 日历显示完成进度**：状态 Tag 增加 `已完成/总数`（如 `1/3`）；配合后端部分完成状态为橙色。
...
- **门店考勤设置**：Stores「考勤设置」Tab——「要求打卡」「屏蔽公共假期」。
- **屏蔽公共假期语义**：开启后隐藏/忽略已配置公共假期（排班不高亮、详情不展示）；配置数据保留，关闭即可恢复。不禁止排班、不影响打卡。
- **今日出勤**：支持后端 `punch_exempt`（免打卡）状态展示。


## 2026-07-15

- **简易版控制台入口横向排列**：快捷入口改为固定宽度长方形主色实心按钮（`type="primary"`），图标与文字居中，横向 `flex-wrap` 排列；有二级权限仍下拉选择后再进入。
  - **`SimpleHomeLauncher.tsx`**、**`locales.ts`**。
